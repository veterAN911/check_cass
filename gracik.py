...
def compare_receipts_in_shift(
    ...
    try:
        ...
        messagebox.showerror("Error", f"Произошла ошибка при подключении к ОФД")
        print(f"Ошибка: {str(e)}nn{traceback.format_exc()}")        
    list1 = []
    ...
    if (missing_elements == []):
        ...
        if answer == "yes":
            set_text_to_entry_logi("\nИдут процесс создания чеков ожидайте(может занять до нескольких минут)")
...
label_version = tk.Label(container, text="version 1.1 / 2024 г")
