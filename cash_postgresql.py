...
def new_cap_check(
        ...
        data_position):
    cursor = conn.cursor()
    zapros_fool_check = f"SELECT numberfield,inn,kpp,id_session FROM ch_purchase WHERE id_shift = '{id}' AND fiscal_doc_id = '{
        fiscal_num - 1}'"
    cursor.execute(zapros_fool_check)
    general_info = cursor.fetchall()
    if general_info == []:
        zapros_fool_check = cursor.execute(
            "SELECT numberfield, inn, kpp, id_session FROM ch_purchase ORDER BY id DESC LIMIT 1")
        general_info = cursor.fetchall()
        general_info = [
            (0,
             general_info[0][1],
